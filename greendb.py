...
class ProtocolHandler:
    ...
    def handle_processing_instruction(self, sock):
        ...
        if instruction not in PROCESSING_INSTRUCTIONS:
            raise ValueError(
                "unrecognized processing instruction, refusing to" " process."
            )
...
class Storage:
    ...
    def open(self):
        ...
        for i in range(self._config["max_dbs"]):
            ...
            self.databases[i] = self.env.open_db(
                encode("db%s" % i), dupsort=self.supports_dupsort(i)
            )
    ...
    def db(self, db=0, write=False):
        ...
        txn = self.env.begin(db=self.databases[db], write=write, buffers=use_buffers)
        try:
            yield txn
...
def requires_dupsort(meth):
    ...
    def verify_dupsort(self, client, *args):
        if not self.storage.supports_dupsort(client.db):
            raise CommandError(
                "Currently-selected database %s does not "
...
class Server:
    def __init__(
        ...
        **storage_config,
        ...
        self._server = StreamServer(
            (self._host, self._port), self.connection_handler, spawn=self._pool
        )
    ...
    def setnx(self, client, key, value):
        with client.ctx(True) as txn:
            return txn.put(encode(key), mpackb(value), dupdata=False, overwrite=False)
    ...
    def msetnx(self, client, data):
        with client.cursor(True) as cursor:
            consumed, added = cursor.putmulti(
                mpackdict(data), dupdata=False, overwrite=False
            )
        return added
    ...
    def connection_handler(self, conn, address):
        logger.info(f"{server._host} {server._port}: Connection received: {address}")
...
class _ConnectionLocal(_ConnectionState, greenlet_local):
    pass
...
def get_option_parser():
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="map_async",
        help=('When used with "--writemap" (-W), use asynchronous ' "flushes to disk."),
    )
    return parser
...
def parse_map_size(value):
    ...
    if not mapsize.isdigit():
        raise ValueError(
            'cannot parse file-size "%s", use a valid file-'
